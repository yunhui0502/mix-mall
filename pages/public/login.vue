			...
			<view class="input-content">
				...
				<view class="input-item">
					...
					<view style="display:flex;justify-content:space-between;width: 100%;">
						...
						<view @click="code" style="font-size: 26upx;width: 36%;display:flex;justify-content : flex-end;">获取验证码</view>
					</view>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			code(){
			  userApi.code({phone:this.mobile},(res)=>{
				  console.log(res)
			  })
			},
			navBack() {
			...
			async toLogin() {
				...
				userApi.login({authKey:this.mobile,bossId:this.$store.state.bossId,passwd:this.password},(res)=>{
					...
					if(res.data.status==200){
						if(res.data.data=='验证码不正确'){
						 ...
						 uni.setStorageSync('data',res.data.data);
						 console.log(uni.getStorageSync('data').token)
						uni.navigateBack();
						// uni.switchTab({
