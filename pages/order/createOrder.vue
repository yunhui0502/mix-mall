<template>
	<view>
			...
			<view class="order-content">
				...
				<view class="cen">
					<view class="top">
						...
						<text class="mobile">{{addressData.address}}</text>
					</view>
					<view class="central">
						<text class="mobile">{{addressData.address}}</text>
					</view>
					<text class="address">{{addressData.name}} {{addressData.mobile}}</text>
				</view>
		...
		<view class="quhuomethod" style="margin-top:50rpx">
			...
			<text @click="changeMethod" class="ziti">{{pickUp.wayOfPickUpDesc}}</text>
			<text style="width:12rpx;height:22rpx" @click="changeMethod" class="yticon icon-you"></text>
		</view>
		...
		<view class="paymentMethod">
			<radio-group class="item-right radio-group" v-for="(item,index) in paymentMethod" :key="index" @change="onPaymentMethod">
				<label class="radio">
		...
		<view class="goods-section">
			<view class="g-header b-b">
				...
				<text class="name">{{goods.bossName}}</text>
			</view>
			<!-- 商品列表 -->
			<view class="g-item">
				<image src="https://ss2.bdstatic.com/70cFvnSh_Q1YnxGkpoWK1HF6hhy/it/u=756705744,3505936868&fm=11&gp=0.jpg"></image>
				<view class="right">
					<text class="title clamp">{{goods.productDesc}}</text>
					<text class="spec">{{goods.hfNames}}</text>
					<view class="price-box">
						<text class="price">￥{{goods.priceArea}}</text>
						<text class="number">x {{goodsList[0].quantity}}</text>
					</view>
				</view>
			</view>
			<view class="g-item">
...
<script>
	...
	export default {
		...
		onLoad(option) {
			 ...
			 this.goods =  this.goodsList[0].goods
			//商品数据
		...
		methods: {
			onPaymentMethod(e) {
				...
				for (let payment of this.paymentMethod) {
			...
			//切换提货方式
			changeMethod() {
				if (this.pickUp.wayOfPickUp == 'selfPickUp') {
				...
				} else if (this.pickUp.wayOfPickUp == 'delivery') {
			...
			submit() {
				...
				let params = {
					...
					userId: uni.getStorageSync('data').userId,
					// disconuntId: this.data.disconuntId, //优惠券
					orderType: 'nomalOrder',
					paymentName: this.paymentMethod[0].name, // 支付方式
					hfRemark: this.desc,
					// activityId: this.data.activityId,
					takingType: this.pickUp.wayOfPickUp,//是不是发快递
				};
				// if (typeof(this.data.selectedAddress.id) != 'undefined') {
				// 	params.userAddressId = this.data.selectedAddress.id;  //邮寄地址id
				// }
